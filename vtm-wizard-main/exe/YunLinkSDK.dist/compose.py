...
{\n\
    ...
    print("V1.35 compose success !")


def getfiles(argv, mode):
    files = []
    for demo in argv:
        print('./xml/USBV1.0/' + demo + '/filenam   e.xml')
        ...
        for filename in file:
            if filename.firstChild.data not in files:
                ...
                print(filename.firstChild.data + "\t\t\tloading")
    print("模块文件列表加载完毕\n\n")
    return files
...
def project_cp(path, uvprojxmap, demo):
    ...
    try:
        if 'CDC_HID_Composite' == demo:
            ...
            fcode = fcode.replace(str1, str2)
            # print(fcode)
            cf = open(path + "/out/USBV1.0/" + args.o + "/Projects/" + args.o + "/" + demo + "/MDK-ARM/" + uvprojxmap[demo], "w")
            cf.write(fcode)
            cf.close()
        else:
